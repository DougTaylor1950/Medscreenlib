...
#End Region
        ...
        Try
            ...
            If myTable Is Nothing Then
                ...
                If myTable.Name = "" Then
                    myTable = Nothing
                    Medscreen.LogAction("Table has no name")
                Else
                    Tables.Add(myTable)
                End If
            End If
  ...
  Public Shared Function GetTableName(ByVal activeTable As String, ByVal tableDate As Date) As String
    ...
    Try
      ...
      createView.CommandText = "SELECT COUNT(1) FROM User_Tables " & _
            ...
            createView.Parameters.AddWithValue("Table_Name", "C_" & activeTable.ToUpper & "_" & yearString.Substring(1))
  ...
  Public Shared Function TableExists(ByVal tableName As String) As Boolean
    ...
    Dim findTable As New OleDb.OleDbCommand("SELECT Table_Name FROM User_Tables " & _
        ...
        findTable.Parameters.AddWithValue("Table_Name", tableName.ToUpper)
    Try
      MedConnection.Open()
  ...
  Public Shared Function ViewExists(ByVal viewName As String) As Boolean
    ...
    Dim findTable As New OleDb.OleDbCommand("SELECT View_Name FROM User_Views " & _
        ...
        findTable.Parameters.AddWithValue("View_Name", viewName.ToUpper)
    Try
      MedConnection.Open()
      exists = Not findTable.ExecuteScalar Is Nothing
    Catch ex As Exception
  ...
  Public Shared Function GetCommittedTables(ByVal activeTable As String, ByVal includeActive As Boolean, ByVal minYear As Integer) As String()
    ...
    Try
      listTables.CommandText = "SELECT Table_Name, GetTableYear(Table_Name) Year FROM User_Tables" & _
            ...
            listTables.Parameters.AddWithValue("Table_Name", "C_" & activeTable.ToUpper & "____")
            listTables.Parameters.AddWithValue("Year", minYear)
      MedConnection.Open()
...
Public Class StructTable
    ...
    Friend Sub New(ByVal definition As String, ByVal text As String, ByRef startPos As Integer)
      ...
      ' Find the name of this table
            ...
            Medscreen.LogAction("After parser " & _name & " | " & definition)
    ...
    Friend Sub New(ByVal tableName As String)
      Try
        tableName = tableName.ToUpper
                ...
                getComment.Parameters.AddWithValue("Table", tableName)
                Try
                    MedConnection.Open()
    ...
    Public Function GetFields(ByVal fields As IEnumerable) As StructField.FieldCollection
            Dim field As Object, myFields As New StructField.FieldCollection()
            For Each field In fields
                Dim strFieldname As String
                If TypeOf field Is String Then
                    strFieldname = CStr(field)
                ElseIf TypeOf field Is StructField Then
                    strFieldname = CType(field, StructField).Name
                Else
                    strFieldname = CStr(field)
                End If
                If Not Me.Fields(strFieldname) Is Nothing Then
                    myFields.Add(Me.Fields(strFieldname))
                End If
            Next
      Return myFields
    End Function
    ...
    End Function
        ...
        Public Function GetBrowseCommand(ByVal allowRemoved As Boolean) As OleDb.OleDbCommand
            Return Me.GetBrowseCommand(allowRemoved, Nothing)
        End Function



        Public Function GetBrowseCommand(ByVal allowRemoved As Boolean, ByVal filter As String) As OleDb.OleDbCommand
            Dim browseCmd As New OleDb.OleDbCommand("SELECT * FROM " & Me.Name, MedscreenLib.MedConnection.Connection)
            If filter > "" Or Not allowRemoved Then
                If Not allowRemoved Then
                    Dim removeField As StructField = Me.GetFieldFor(FieldUsage.RemoveFlag)
                    If Not removeField Is Nothing Then
                        browseCmd.CommandText &= " WHERE " & removeField.Name & " = ?"
                        browseCmd.Parameters.Add("Removed", OleDb.OleDbType.VarChar).Value = MedscreenLib.C_dbFALSE
                        If filter > "" Then
                            browseCmd.CommandText &= " AND "
                        End If
                    End If
                End If
                If filter > "" Then
                    If browseCmd.CommandText.IndexOf("WHERE") < 0 AndAlso filter.IndexOf("WHERE") < 0 Then
                        browseCmd.CommandText &= " WHERE "
                    End If
                    browseCmd.CommandText &= filter
                End If
            End If
            Dim orderField As StructField = Me.GetFieldFor(FieldUsage.Ordering)
            If Not orderField Is Nothing Then
                browseCmd.CommandText &= " ORDER BY " & orderField.Name
            End If
            Return browseCmd
        End Function
    ''' -----------------------------------------------------------------------------
