...
Public Class WebCacheElement
    ...
    Private myReflectionPath As String
    Public Property ReflectionPath() As String
        Get
            Return myReflectionPath
        End Get
        Set(ByVal value As String)
            myReflectionPath = value
        End Set
    End Property
    ...
    Public ReadOnly Property HyperlinkXML() As String
        Get
            Return Medscreen.ReplaceString(Me.HyperLink, "&", "&&")
        End Get
    End Property

    Private myTimeStamp As DateTime
...
Public Class WebCache
    ...
    Default Public Property Item(ByVal Index As String, ByVal indexby As IndexBy) As WebCacheElement
        Get
            Dim objWE As WebCacheElement = Nothing
            Dim i As Integer = 0
    ...
    ''' </history>
    ''' -----------------------------------------------------------------------------
    Public Overloads Function Add(ByVal Hyperlink As String, ByVal Text As String) As WebCacheElement
        'Dim i As Integer
        Dim objCacheElement As WebCacheElement
        ...
        'Next
        objCacheElement = Item(Hyperlink, IndexBy.Hyperlink)
        If objCacheElement Is Nothing Then
            Dim objWebCacheElement As New WebCacheElement(Hyperlink, Text)
            MyBase.InnerList.Add(objWebCacheElement)
            myPosition = Count - 1
            objCacheElement = objWebCacheElement
        Else
            myPosition = MyBase.InnerList.IndexOf(objCacheElement)
        End If
        RaiseEvent CacheChanged()
        Return objCacheElement
    End Function

    Public Overloads Function Add(ByVal CacheElement As WebCacheElement) As WebCacheElement
        MyBase.InnerList.Add(CacheElement)
        myPosition = Count - 1
        RaiseEvent CacheChanged()
        Return CacheElement
    End Function
